...
@Component({
  ...
  template: `
    <div class="min-h-full bg-[var(--app-bg)]">
      <main class="mx-auto w-full px-4 py-4 md:px-6 md:py-5" [style.max-width]="maxWidth">
        <app-breadcrumb [items]="breadcrumbs" />

        @if (title || subtitle) {
          <header class="mb-4 flex flex-col gap-3 border-b border-[var(--app-border)] pb-4 md:flex-row md:items-end md:justify-between">
            <div class="min-w-0">
              @if (eyebrow) {
                <p class="text-xs font-semibold uppercase tracking-wide text-[var(--app-primary)]">
                  {{ eyebrow }}
                </p>
              }
              ...
              @if (subtitle) {
                <p class="mt-1 max-w-3xl text-sm text-[var(--app-text-muted)]">
                  {{ subtitle }}
                </p>
              }
            </div>
